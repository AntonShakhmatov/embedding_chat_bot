...
class EmbeddingsPresenter extends Presenter 
{
    ...
    // public function actionControl($text)
    // {
    //     $queryEmbedding = $this->embeddingsService->get_embedding([$text])[0];
    
    //     $allEmbeddings = $this->embeddingsService->getAllEmbeddings();
    
    //     $results = [];
    //     foreach ($allEmbeddings as $item) {
    //         $embedding = json_decode($item['union_date'], true);
    //         if (!$embedding || !is_array($embedding)) continue;
    //         $similarity = $this->embeddingsService->cosine_similarity($queryEmbedding, $embedding);
    //         $results[] = [
    ...
    //     $this->sendJson([
    //         'result' => array_slice($results, 0, 5)
    //     ]);
    // }
    ...
    //přiklad jen pro description
    // public function actionControl($text)
    // {
    //     // Embedding chat botu
    //     $queryEmbedding = $this->embeddingsService->get_embedding([$text])[0];

    //     // Všechny embeddingy databaze
    //     $allEmbeddings = $this->embeddingsService->getAllEmbeddings();
    ...
    //         $embedding = $data['description'];

    //         if (count($queryEmbedding) !== count($embedding)) {
    //             continue;
    //         }
